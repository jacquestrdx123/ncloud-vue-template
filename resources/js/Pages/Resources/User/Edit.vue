<template>
  <div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">{{ title || 'Edit User' }}</h1>
      </div>
      <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg">
        <form @submit.prevent="submit" class="p-6">
          ...
          <div class="mt-6 flex items-center justify-end gap-3">
            <Link :href="route('users.index')" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600">
              Cancel
            </Link>
            <button
              type="submit"
...
props.fields.forEach(field => {
  formData[field.name] = props.item[field.name] || field.default || field.value || null
})
...
const submit = () => {
  ...
  form.put(route('users.update', itemId), {
      preserveScroll: true
    })
}
</script>
